...
<template>
  <main>
    ...
    <div class="prog">
      <progress :max="todoStore.todos.length" :value="todoStore.completedTodos.length"></progress>
      <p>
    ...
    <ul class="todos" v-for="todo in todoStore.todos" v-bind:key="todo.Id">
      <li class="todo">
        <input type="checkbox" name="isCompleted" :id="todo.Id" :checked="todo.Completed"
          @change="todoStore.toggleTodo(todo.Id)" />
        <label :for="todo.Id" :class="todo.Completed ? 'completed' : 'incomplete'">{{ todo.Description }}</label>
      </li>
    </ul>
